...
async function main() {
  ...
  // Pacientes (3 pacientes adicionales)
  const patientsData = [
    {
  ...
  // 2. CREAR SERVICIOS (10 servicios)
  ...
  // 3. CREAR HORARIOS MÉDICOS (DoctorSchedule)
  ...
  // 4. CREAR SERVICIOS MÉDICOS (DoctorService)
  console.log('🔗 Vinculando servicios con doctores...');

  let doctorServiceCount = 0;
  for (const doctor of doctors) {
    // Cada doctor ofrece 3-5 servicios aleatorios
  ...
  // 5. CREAR CITAS (5 citas adicionales)
    ...
    if (appointment && appointment.status === 'COMPLETED') {
      await prisma.review.create({
        data: {
          ...
          appointmentId: appointment.id,
          rating: 4 + Math.floor(Math.random() * 2), // 4 o 5
  ...
  // 8. CREAR HISTORIAS MÉDICAS (MedicalRecords)
      ...
      for (let i = 0; i < numRecords; i++) {
        await prisma.medicalRecord.create({
          data: {
            ...
            type: recordTypes[i % recordTypes.length] as any,
  ...
  // 9. CREAR PRESCRIPCIONES (Prescriptions)
  ...
  for (const patient of patients) {
    if (patient) {
      // 1-2 prescripciones por paciente
  ...
  // 11. CREAR CONFIGURACIONES (Settings)
